...
string err_msg = "Pass <-m peer1 peer2> arguments for man in the middle and <-s> for server and <-c name_of_host> for client";
...
int create_client_socket(char* name_of_host, int port) {
	int s;
	// declaring address for server
	struct sockaddr_in addr;

	// get hostent structure from name_of_host
	struct hostent *host = gethostbyname(name_of_host);
	...
	cout<<"Connected to "<<name_of_host<<"...\n";
	return s;
}
...
void configure_context(SSL_CTX* ctx) {
	...
	if(!SSL_CTX_load_verify_locations(ctx, "CAfile.pem", NULL))
	{
...
int main(int argc, char** argv) {
	...
	char* name_of_host; // host name of server which client desires to connect
	int n, bytes_read;
	...
	if(flag == 0) {
		...
		int alice_socket, bob_socket, sock;
		...
		alice_socket = accept(sock, (struct sockaddr*)&client_addr, &len);	
		if(alice_socket < 0) {
		...
		bob_socket = create_client_socket(peer2, SERVER_LISTEN_PORT);

		// Trudy receives "chat_hello" from alice
		bzero(alice_msg, BUF_SIZE);
		bytes_read = read(alice_socket, alice_msg, BUF_SIZE);
		if(bytes_read > 0) {
		...
		write(bob_socket, alice_msg, BUF_SIZE);

		// Trudy receives "chat_reply" from Bob
		bzero(bob_msg, BUF_SIZE);
		bytes_read = read(bob_socket, bob_msg, BUF_SIZE);
		if(bytes_read > 0) {
		...
		write(alice_socket, bob_msg, BUF_SIZE);
        ...
        write(alice_socket, alice_msg, BUF_SIZE);
		...
		// creating socket 
		int sock = create_client_socket(name_of_host, MITM_LISTEN_PORT);
		...
		SSL_set_fd(alice_ssl, alice_socket);

		// setting the file descriptor of bob socket with bob ssl
		SSL_set_fd(bob_ssl, bob_socket);
